@extends('layouts.base')
@section('content')
<div class="col-sm-12">
    <div class="card">
        <div class="card-header">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div class="float-right">
                    <img src="data:image/png;base64,"/><br>
                    <a class="btn btn-primary" href="{{ route('dashboard') }}">Home</a>
                </div>
                <span id="card_title" style="color:#82adfd; font-size: 50px">
                    {{ __('Albums') }}
                </span>
                <div class="float-right">
                    @can('is-admin', Auth::user())
                        <a href="{{ route('albums.create') }}" class="btn btn-primary btn-sm float-right"  data-placement="left">
                            {{ __('Create New Album') }}
                        </a>
                        <img src="data:image/png;base64,"/>
                    @endcan
                        <div>
                            <form action="{{route('albums.index')}}" method="get">
                                <input type="search" name="search" placeholder="Search album" style="height: 50px; width: 200px">
                                <input type="submit" value="Go !" style="height: 50px; width: 200px">
                            </form>
                        </div>
                </div>
            </div>
        </div>
        @if ($message = Session::get('success'))
            <div class="alert alert-success">
                            <p>{{ $message }}</p>
                        </div>
        @endif
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    <thead class="thead">
                    <tr>
                        <th>Albums Titles</th>
                        <th>Artists</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($albums as $album)
                        <tr>
                            <td>{{ ucfirst($album->title) }}</td>
                            <td>{{ ucfirst($album->artist->name) }}</td>
                            <td>
                                <form action="{{ route('albums.destroy',$album->id) }}" method="POST">
                                    <a class="btn btn-sm btn-primary " href="{{ route('albums.show',$album->id) }}"><i class="fa fa-fw fa-eye"></i> Show</a>
                                    @can('is-admin', Auth::user())
                                        <a class="btn btn-sm btn-success" href="{{ route('albums.edit',$album->id) }}"><i class="fa fa-fw fa-edit"></i> Edit</a>
                                    @endcan
                                    @csrf
                                    @can('is-admin', Auth::user())
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm"><i class="fa fa-fw fa-trash"></i> Delete</button>
                                    @endcan
                                </form>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
            </div>
        </div>
    </div>
    {!! $albums->links() !!}
</div>
    <a href="{{ route('albums.export') }}">Export</a>
@endsection
