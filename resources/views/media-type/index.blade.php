@extends('layouts.base')

@section('template_title')
    Media Type
@endsection

@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div class="float-right">
                                <img src="data:image/png;base64,"/><br>
                                <a class="btn btn-primary" href="{{ route('dashboard') }}">Home</a>
                            </div>
                            <span id="card_title" style="color:#82adfd; font-size: 50px">
                                {{ __('Media Types') }}
                            </span>

                             <div class="float-right">
                                <a href="{{ route('media-types.create') }}" class="btn btn-primary btn-sm float-right"  data-placement="left">
                                  {{ __('Create New Media Type') }}
                                </a>
                                 <img src="data:image/png;base64,"/>
                              </div>
                        </div>
                    </div>
                    @if ($message = Session::get('success'))
                        <div class="alert alert-success">
                            <p>{{ $message }}</p>
                        </div>
                    @endif

                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                <thead class="thead">
                                    <tr>
                                        <th>No</th>

										<th>Name</th>

                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($mediaTypes as $mediaType)
                                        <tr>
                                            <td>{{ ++$i }}</td>

											<td>{{ $mediaType->name }}</td>

                                            <td>
                                                <form action="{{ route('media-types.destroy',$mediaType->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('media-types.show',$mediaType->id) }}"><i class="fa fa-fw fa-eye"></i> Show</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('media-types.edit',$mediaType->id) }}"><i class="fa fa-fw fa-edit"></i> Edit</a>
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-sm"><i class="fa fa-fw fa-trash"></i> Delete</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
                {!! $mediaTypes->links() !!}
            </div>
        </div>
    </div>
@endsection
