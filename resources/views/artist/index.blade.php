@extends('layouts.base')

@section('template_title')
    Artist
@endsection

@section('content')
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div class="float-right">
                        <img src="data:image/png;base64,"/><br>
                        <a class="btn btn-primary" href="{{ route('dashboard') }}">Home</a>
                    </div>
                    <span id="card_title" style="color:#82adfd;font-size: 50px;">
                        {{ __('Artists') }}
                    </span>
                    <div class="float-right">
                        @can('is-admin', Auth::user())
                            <a href="{{ route('artists.create') }}" class="btn btn-primary btn-sm float-right"  data-placement="left">
                                {{ __('Create New Artist') }}
                            </a>
                            <img src="data:image/png;base64,"/>
                        @endcan
                            <div>
                                <form action="{{route('artists.index')}}" method="get">
                                    <input type="search" name="search" placeholder="Search artist" style="height: 50px; width: 200px">
                                    <input type="submit" value="Go !" style="height: 50px; width: 200px">
                                </form>
                            </div>
                    </div>
                </div>
            </div>
            @if ($message = Session::get('success'))
                <div class="alert alert-success">
                    <p>{{ $message }}</p>
                </div>
            @endif

            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        <thead class="thead">
                        <tr>
                            <th>ID</th>
                            <th>Name</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach ($artists as $artist)
                            <tr>
                                <td>{{ ucfirst($artist->id)}}</td>
                                <td>{{ ucfirst($artist->name)}}</td>
                                <td>
                                    <form action="{{ route('artists.destroy',$artist->id) }}" method="POST">
                                        <a class="btn btn-sm btn-primary " href="{{ route('artists.show',$artist->id) }}"><i class="fa fa-fw fa-eye"></i> Show</a>
                                        @can('is-admin', Auth::user())
                                            <a class="btn btn-sm btn-success" href="{{ route('artists.edit',$artist->id) }}"><i class="fa fa-fw fa-edit"></i> Edit</a>
                                        @endcan
                                        @csrf
                                        @method('DELETE')
                                        @can('is-admin', Auth::user())
                                            <button type="submit" class="btn btn-danger btn-sm"><i class="fa fa-fw fa-trash"></i> Delete</button>
                                        @endcan
                                    </form>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
        {!! $artists->links() !!}
    </div>
@endsection
