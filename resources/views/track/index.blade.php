@extends('layouts.base')

@section('template_title')
    Track
@endsection

@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div class="float-right">
                                <img src="data:image/png;base64,"/><br>
                                <a class="btn btn-primary" href="{{ route('dashboard') }}">Home</a>
                            </div>
                            <span id="card_title" style="color:#82adfd; font-size: 50px">
                                {{ __('Tracks') }}
                            </span>

                             <div class="float-right">
                                 @can('is-admin', Auth::user())
                                <a href="{{ route('tracks.create') }}" class="btn btn-primary btn-sm float-right"  data-placement="left">
                                  {{ __('Create New Track') }}
                                </a>
                                 <img src="data:image/png;base64,"/>
                                 @endcan
                                     <div>
                                         <form action="{{route('tracks.index')}}" method="get">
                                             <input type="search" name="search" placeholder="Search tracks" style="height: 50px; width: 200px">
                                             <input type="submit" value="Go !" style="height: 50px; width: 200px">
                                         </form>
                                     </div>
                              </div>
                        </div>
                    </div>
                    @if ($message = Session::get('success'))
                        <div class="alert alert-success">
                            <p>{{ $message }}</p>
                        </div>
                    @endif

                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                <thead class="thead">
                                    <tr>
										<th>Name</th>
										<th>Album Name</th>
										<th>Media Type</th>
										<th>Genre</th>
										<th>Composer</th>
										<th>Unit Price</th>
                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($tracks as $track)
                                        <tr>
											<td>{{ucfirst($track->name)}}</td>
											<td>{{ucfirst( $track->album?->title ?? 'aucun album')}}</td>
											<td>{{$track->mediaType->name}}</td>
											<td>{{ucfirst($track->genre?->name ?? 'aucun genre')}}</td>
											<td>{{ucfirst($track->composer)}}</td>
											<td>{{ $track->unit_price }}€</td>

                                            <td>
                                                <form action="{{ route('tracks.destroy',$track->id) }}" method="POST">
                                                <a class="btn btn-sm btn-primary " href="{{ route('tracks.show',$track->id) }}"><i class="fa fa-fw fa-eye"></i> Show</a>
                                                @can('is-admin', Auth::user())
                                                    <a class="btn btn-sm btn-success" href="{{ route('tracks.edit',$track->id) }}"><i class="fa fa-fw fa-edit"></i> Edit</a>
                                                @endcan
                                                @csrf
                                                @can('is-admin', Auth::user())
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-sm"><i class="fa fa-fw fa-trash"></i> Delete</button>
                                                @endcan
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
                {!! $tracks->links() !!}
            </div>
        </div>
    </div>
@endsection
